...
drawHUDEnd = () => {
    ...
    ctx.fillText("Mungkin gamenya kegampangan buat kamu!", canvasHUD.width / 2, canvasHUD.height / 2 - 100);
...
drawHUDHome = () => {
    ...
    ctx.fillText("Tekan tombol A untuk mulai", canvasHUD.width / 2, canvasHUD.height / 2 + 100);
}
drawHUD = () => {
    ctx.font = 30 + 'px MyCustomFont';
    if (
    ...
    // );
    {
        ...
        ctx.textAlign = "center";
        ctx.font = 30 + 'px MyCustomFont';
        ctx.fillText("Level " + gameLevel + " / " + NUMBER_OF_LEVEL, canvasHUD.width / 2, 50);
        if (!isShowText)
            return;
        ctx.textAlign = "left";
        ctx.font = 20 + 'px MyCustomFont';
        ctx.fillStyle = "#ffffff";
        ctx.fillText("Langkah: " + nbMoves, 20, 50);
        ctx.fillText("M - Matikan suara", 20, 90);
        ctx.fillText("R - Ulang", 20, 130);
        ctx.fillText("N - Level berikutnya", 20, 170);
        ctx.fillText("P - Level sebelumnya", 20, 210);
        ctx.fillStyle = "cyan";
        ctx.fillText("H - Buat level sendiri", 20, 250);
        ctx.fillStyle = "#ffffff";
        ctx.fillText("K - Sembunyikan teks", 20, 290);
